...
const EditSlide: React.FC<ProfileInfoProps> = (props) => {
			...
			if (props.dataEdit.id === 0) {
				...
				files?.map(async (item: any) => {
					item.image?.map(async (itemChildren: any) => {
						...
						formData.append('id_product', data.idProduct);
						const responseNewsPost = await CreateSlidePost(formData);
						if (responseNewsPost) {
				...
				files?.map(async (item: any) => {
					if (item.image?.length === 1) {
						item.image?.map(async (itemChildren: any) => {
							...
							formData.append('image', itemChildren);
							formData.append('id_product', data.idProduct);
							formData.append('id_slide', props.dataEdit.id);

							const responseNewsPost = await UpdateSlidePost(formData);
							if (responseNewsPost) {
								if (responseNewsPost.errorCode === null) {
									Swal.fire({
										icon: 'success',
								...
								<Grid container spacing={3}>
									...
									<Grid item xs={12}>
										...
										<Controller
											control={control}
											name="idProduct"
											defaultValue={props.dataEdit.idProduct}
